...
- **Part 2 — turn this into a crawlable decision tree: PLANNED, not built.** The roadmap is `Crawlable_Roadmap.md`. Nothing executable (no `evaluate` expressions, no crawler, no fact registry) exists yet — the current artifacts are a faithful *representation*, not a runnable graph.
- **▶ To start Part 2 in a fresh chat, read §9 of this file** — it pins the entry point, the canonical node schema to copy, and the exact output layout so separate chats don't diverge.
